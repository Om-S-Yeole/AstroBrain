...
class ThermalConfig(TypedDict):
    mass_kg: float
    heat_capacity_J_per_kgK: float
    area_m2: float
    absorptivity: float  # α
    emissivity: float  # ε
    Tmin_K: float
    Tmax_K: float
...
class ThermalResults(TypedDict):
    min_temp_K: float
    max_temp_K: float
    thermal_violation: bool


def compute_solar_heat_input(
    beta_deg: float,
...
def compute_radiation_heat(temp_K: float, thermal_config: ThermalConfig):
    """
