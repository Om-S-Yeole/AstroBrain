...
class ComputeDataGeneratedSchema(BaseModel):
    model_config = {"arbitrary_types_allowed": True}
    on_time_s: float
    data_rate_Mbps: float


class ComputeDataDownlinkedSchema(BaseModel):
    model_config = {"arbitrary_types_allowed": True}
    communication_results: CommunicationResults
    downlink_rate_Mbps: float


class EvaluateDutyCycleSchema(BaseModel):
    model_config = {"arbitrary_types_allowed": True}
...
@tool(args_schema=ComputePayloadONTimeSchema)
def compute_payload_ON_time_tool(times: list | np.ndarray, duty_cycle: float) -> float:
    """
...
@tool(args_schema=ComputeDataGeneratedSchema)
def compute_data_generated_tool(on_time_s: float, data_rate_Mbps: float) -> float:
    """
...
@tool(args_schema=ComputeDataDownlinkedSchema)
def compute_data_downlinked_tool(
...
@tool(args_schema=EvaluateDutyCycleSchema)
def evaluate_duty_cycle_tool(
    propagation_results: PropagationResults,
    eclipse_results: EclipseResults,
    power_results: PowerResults,
    communication_results: CommunicationResults,
