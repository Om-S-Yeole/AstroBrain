...
class ComputeVisibilityForStationToolSchema(BaseModel):
...
class ComputeCommunicationToolSchema(BaseModel):
    model_config = {"arbitrary_types_allowed": True}
    propagation_results: PropagationResults
    ground_stations: list[GroundStationConfig]


@tool(args_schema=ComputeVisibilityForStationToolSchema)
def compute_visibility_for_station_tool(
    propagation_results: PropagationResults,
    ground_station: GroundStationConfig,
) -> StationResult:
    """
...
@tool(args_schema=ComputeContactDurationFromWindowsToolSchema)
def compute_contact_duration_from_windows_tool(windows: list | np.ndarray):
    """
...
@tool(args_schema=ComputePassesToolSchema)
def compute_passes_tool(windows: list | np.ndarray):
    """
...
@tool(args_schema=AggregateGroundStationsToolSchema)
def aggregate_ground_stations_tool(
    station_results: list[StationResult],
) -> CommunicationResults:
    """
...
@tool(args_schema=ComputeCommunicationToolSchema)
def compute_communication_tool(
    propagation_results: PropagationResults,
    ground_stations: list[GroundStationConfig],
) -> CommunicationResults:
    """
