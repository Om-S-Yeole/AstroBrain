...
class ToolSeqList(BaseModel):
    ...
    Wraps a list of tool invocation specifications for power,
    communication, thermal, and duty-cycle analysis.
...
def power_comm_thermal_duty(state: MissionOpsState, config: RunnableConfig):
    """.
    Plan tool invocations for power, communication, thermal, and duty-cycle analysis.

    This node operates after orbit propagation, visibility, sun geometry, and
    eclipse analysis have completed. It uses an LLM to plan the sequence of
    tools needed for resource and constraint analysis to assess mission
    feasibility in terms of power, communications, thermal viability, and
    payload duty cycles.
    ...
    state : MissionOpsState
        ...
        - tool_sequence : list
            Previously planned tool invocations (including outputs from
            earlier analysis stages)
    config : RunnableConfig
        LangGraph configuration containing:
        - configurable["power_comm_thermal_duty_model"] : BaseChatModel
    ...
    dict
        ...
        - tool_sequence : list
            Updated tool sequence with new power/comm/thermal/duty tools appended
        - start_execution_from_idx : int
            Index from which to start executing new tools
        - tool_selection_state_done : str
            Set to "power_comm_thermal_duty" to indicate this planning stage is complete

    Notes
    -----
    This function only plans tools related to electrical power generation/consumption,
    battery modeling, communication access windows, thermal envelope checks, and
    payload duty-cycle validation. It does not execute any tools. Each planned
    tool receives a unique ID generated via uuid_generator_tool. If no resource
    analysis tools are required, an empty sequence is returned.
    """
    model: BaseChatModel = config["configurable"][
        "power_comm_thermal_duty_model"
    ]  # NOTE: This model must have tools binded to it
    structured_model = model.with_structured_output(ToolSeqList)

    # --- SYSTEM PROMPT ----
    # Write the best system prompt for me
...
for POWER, COMMUNICATION, THERMAL, and PAYLOAD DUTY-CYCLE ANALYSIS ONLY.
...
1. Examine:
   ...
   - retrieved reference material
   - outputs or planned outputs from previous tools
   - previously planned tools

2. Determine which RESOURCE and CONSTRAINT ANALYSIS steps
   are REQUIRED to assess mission feasibility, including:
   - electrical power generation and consumption
   - battery state-of-charge evolution
   - communication access windows
   - thermal viability checks
   - payload duty-cycle feasibility
...
3. Construct a TOOL SEQUENCE where:
   ...
   - Power tools MUST consider eclipse and sun geometry outputs.
   - Communication tools MUST consider visibility outputs.
   - Thermal and duty-cycle tools MUST consider power timelines.
...
5. ONLY plan tools related to:
   - Power generation / consumption / battery modeling
   - Communication window and access analysis
   - Thermal envelope feasibility checks
   - Payload duty-cycle validation
...
- Do NOT plan tools unrelated to power, communication, thermal, or duty-cycle analysis.
...
If NO new power, communication, thermal, or duty-cycle tools
...
"""
    ...
    return {
        ...
        "tool_selection_state_done": "power_comm_thermal_duty",
    }
