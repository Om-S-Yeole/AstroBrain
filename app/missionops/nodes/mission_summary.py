...
class ToolSeqList(BaseModel):
    ...
    Wraps a list of tool invocation specifications for mission
...
def mission_summary(state: MissionOpsState, config: RunnableConfig):
    """.
    Plan final mission-summary tool invocations for result aggregation.

    This node operates after all primary mission analysis stages (orbit
    propagation, visibility, sun geometry, eclipse, power, communication,
    thermal, and duty-cycle analysis) have completed. It uses an LLM to
    plan the final aggregation and summary tools needed to prepare a
    complete mission assessment for user-facing reporting.
    ...
    config : RunnableConfig
        ...
        - configurable["mission_summary_model"] : BaseChatModel
    ...
    dict
        ...
        - tool_sequence : list
            Updated tool sequence with new mission-summary tools appended
        - start_execution_from_idx : int
            Index from which to start executing new tools
        - tool_selection_state_done : str
            Set to "mission_summary" to indicate this planning stage is complete

    Notes
    -----
    This function does not execute tools. It only plans which aggregation
    and summary tools should be invoked to prepare the final mission report.
    Each planned tool receives a unique ID generated via uuid_generator_tool.
    If no mission-summary tools are required, an empty sequence is returned.
    """
    model: BaseChatModel = config["configurable"][
        "mission_summary_model"
...
Your role is to PLAN FINAL MISSION-SUMMARY TOOL INVOCATIONS that AGGREGATE, ORGANIZE, and SUMMARIZE results
from ALL PREVIOUS ANALYSIS STAGES.

You operate AFTER:
- orbit propagation and visibility analysis
- sun geometry and eclipse analysis
- power, communication, thermal, and duty-cycle analysis

This is the FINAL tool-planning stage before user-facing reporting.
...
1. Examine:
   ...
   - retrieved reference material
   - outputs or planned outputs from ALL previous tools
   - previously planned tools

2. Determine which FINAL AGGREGATION and SUMMARY steps
are REQUIRED to prepare a complete mission assessment, including:
   - mission feasibility verdict
   - key performance metrics
   - timelines and windows
   - identified risks and warnings
   - structured data required for final reporting

3. Construct a TOOL SEQUENCE where:
   - Each tool invocation is represented by a ToolData object.
   - Tools ONLY aggregate or summarize existing results.
   - NO new physical analysis tools are planned.
   - Tools reference outputs of previous tools using dict_ref or index_ref.
   - Tools produce clean, structured outputs suitable for final explanation.
...
5. ONLY plan tools related to:
   ...
   - risk and warning aggregation
   - result packaging for final response
...
- Do NOT plan any new physics, orbital, power, or thermal analysis tools.
...
If NO mission-summary tools are required at this stage:
...
"""
    ...
    return {
        ...
        "tool_selection_state_done": "mission_summary",
    }
